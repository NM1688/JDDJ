...
更新地址：https://gitee.com/jiandjh/docker/raw/main/jd/jd_submit_code.js
============Quantumultx===============
[task_local]
#获取互助码
10 0 1/7 * * https://gitee.com/jiandjh/docker/raw/main/jd/jd_submit_code.js, tag=获取互助码,  enabled=true
================Loon==============
[Script]
cron "10 0 1/7 * *" script-path=https://gitee.com/jiandjh/docker/raw/main/jd/jd_submit_code.js,tag=获取互助码
===============Surge=================
获取互助码 = type=cron,cronexp="10 0 1/7 * *",wake-system=1,timeout=3600,script-path=https://gitee.com/jiandjh/docker/raw/main/jd/jd_submit_code.js
============小火箭=========
获取互助码 = type=cron,script-path=https://gitee.com/jiandjh/docker/raw/main/jd/jd_submit_code.js, cronexpr="10 0 1/7 * *", timeout=3600, enable=true
*/
const $ = new Env("获取互助码");
              ...
              if (data.data.bizCode === 0) {
                ...
                $.taskVos.map((item) => {
                  if (item.taskType === 14) {
                    ...
                    sendCode('ddfactory', item.assistTaskDetailVo.taskToken)
                  }
                });
              }
            }
...
function getCash() {
  return new Promise((resolve) => {
    $.get(cashTaskUrl("cash_mob_home",), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          ...
          if (safeGet(data)) {
            ...
            if(data.code === 0 && data.data.result){
              ...
              sendCode('jdcash', data.data.result.inviteCode)
            }
          }
        }
      } catch (e) {
...
function cashTaskUrl(functionId, body = {}) {
  const JD_CASH_API_HOST = 'https://api.m.jd.com/client.action';
  return {
    url: `${JD_CASH_API_HOST}?functionId=${functionId}&body=${escape(JSON.stringify(body))}&appid=CashRewardMiniH5Env&appid=9.1.0`,
    headers: {
      'Cookie': cookie,
      'Host': 'api.m.jd.com',
  ...
  return new Promise(async (resolve) => {
    $.get(taskUrl(`user/QueryUserInfo`), (err, resp, data) => {
      try {
        ...
        sendCode('jxcfd', strMyShareId)
        sendCode('joy', $.UserName)
      } catch (e) {
        $.logErr(e, resp);
      } finally {
        resolve();
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                ...
                if (data.factoryList && data.productionList) {
                  ...
                  sendCode('jxfactory', data.user.encryptPin)
                }
              } else {
      ...
      try {
        if (err) {
          ...
          if (
            ...
            sendCode('pet', $.petInfo.shareCode);
...
async function getJDFruit() {
  ...
  async function jdFruit() {
    ...
    if ($.farmInfo.farmUserPro) {
      ...
      sendCode('farm', $.farmInfo.farmUserPro.shareCode)
    } else {
      /*console.log(
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.success && data.data && data.data.userInviteCode) {
              ...
              sendCode('crazyJoy', data.data.userInviteCode)
            }
          }
        }
      } catch (e) {
      ...
      $.post(url, async (err, resp, data) => {
        try {
          ...
          if (data.data.bizCode === 0) {
            ...
            sendCode('sgmh', `${invites && invites[0]['assistTaskDetailVo']['taskToken']}`)
          }
        } catch (e) {
          $.logErr(e, resp);
        } finally {
...
function sendCode(type, code) {
    return new Promise(async resolve => {
        $.get({ url: `${BASE_URL}/code/add/${type}/${code}`, timeout: 10000 }, (err, resp, data) => {
          try {
            if (err) {
              console.log(`${JSON.stringify(err)}`)
