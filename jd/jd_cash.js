...
const inviteCodes = [
    `eU9YL5XqGLxSmRSAkwxR@eU9YaO7jMvwh-W_VzyUX0Q@eU9YaurkY69zoj3UniVAgg@eU9YaOnjYK4j-GvWmXIWhA@eU9YMZ_gPpRurC-foglg`,
...
function readShareCode() {
    ...
    return new Promise(async resolve => {
        $.get({url: `http://hei.aouy.top:8081/jd/code/read/jdcash/${randomCount}/`, 'timeout': 10000}, (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
